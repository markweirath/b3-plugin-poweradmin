...
#--Commands implementation ------------------------------------------------------------------------
  ...
  def cmd_pamono(self, data, client, cmd=None):
    """\
    <player> - Clear a clients name from colorcodes
    (You can safely use the command without the 'pa' at the beginning)
    """
    # this will split the player name and the message
    input = self._adminPlugin.parseUserCmd(data)
    ...
    # are we still here? Let's write it to console
    self.console.setCvar( 'b3_rmono','%s' % sclient.cid )
  ...
  def cmd_panades(self, data, client, cmd=None):
    """\
    <player> <number> - Grant a player nade(s)
    (You can safely use the command without the 'pa' at the beginning)
    """
    # this will split the player name and the message
    input = self._adminPlugin.parseUserCmd(data)
    if input:
      ...
      client.message('^7Invalid data, try !help panades')
      return False

    if not len(input[1]):
      client.message('^7Missing data, try !help panades')
      return False
    ...
    # are we still here? Let's write it to console
    self.console.setCvar( 'b3_gnammount','%s' % nades )
    self.console.setCvar( 'b3_gnid','%s' % sclient.cid )
